...
class TestBowlingScoreCalculation < Minitest::Test
  ...
  def test_last_frame_is_spare
    shot_marks = '6,3,9,0,0,3,8,2,7,3,X,9,1,8,0,X,6,4,5'
    assert_equal 139, Game.new(shot_marks).score
  end

  def test_last_frame_is_perfect
    shot_marks = '6,3,9,0,0,3,8,2,7,3,X,9,1,8,0,X,X,X,X'
    assert_equal 164, Game.new(shot_marks).score
  end

  def test_last_frame_took_2pitches
    shot_marks = '0,10,1,5,0,0,0,0,X,X,X,5,1,8,1,0,4'
    assert_equal 107, Game.new(shot_marks).score
  end

  def test_last_frame_is_scoreless_after_strikes
    shot_marks = '6,3,9,0,0,3,8,2,7,3,X,9,1,8,0,X,X,0,0'
    assert_equal 134, Game.new(shot_marks).score
  end

  def test_last_frame_is_bonusless_scored_after_strike
    shot_marks = '6,3,9,0,0,3,8,2,7,3,X,9,1,8,0,X,X,1,8'
    assert_equal 144, Game.new(shot_marks).score
  end

  def test_all_strikes_except_the_last
    shot_marks = 'X,X,X,X,X,X,X,X,X,X,X,2'
    assert_equal 292, Game.new(shot_marks).score
  end

  def test_strike_but_no_bonus
    shot_marks = 'X,0,0,X,0,0,X,0,0,X,0,0,X,0,0'
    assert_equal 50, Game.new(shot_marks).score
  end
end
